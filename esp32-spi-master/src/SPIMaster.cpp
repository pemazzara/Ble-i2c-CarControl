...
SPIMaster::SPIMaster() : initialized(false), spi(nullptr) {
    ...
    last_drive_payload.type = CMD_DRIVE; // Por defecto, el tipo de comando es DRIVE
...
bool SPIMaster::performSpiExchange(const ControlCommand_t &cmd){
    //Serial.printf("[SPI] Realizando intercambio SPI para comando: type=%d, speed=%d, angle=%d\n", 
...
bool SPIMaster::requestSensorData() {
    ...
    if (xSemaphoreTake(sensorMutex, pdMS_TO_TICKS(10)) == pdTRUE) {
        ...
        globalSensorData.lastSlaveFlags = stat;
        globalSensorData.lastSlaveState = master_rx_buffer.payload.system.state;
...
bool SPIMaster::processResponse(const SPIResponseFrame_t& response) {
    if (xSemaphoreTake(sensorMutex, pdMS_TO_TICKS(10)) != pdTRUE) return false;

    // Datos comunes a todas las respuestas
    globalSensorData.lastSyncTimestamp = millis();

    if (response.type == TYPE_SENSORS) {
        globalSensorData.sonarDistance = response.payload.motors.distance;
        globalSensorData.a_vel         = response.payload.motors.a_vel;
        globalSensorData.sensorStatus  = response.payload.motors.motor_flags;
        // El estado del esclavo podría venir en un bit de motor_flags o en un campo reservado
        globalSensorData.lastSlaveFlags = response.payload.motors.motor_flags;
        // Si el esclavo no envía explícitamente el estado, lo inferimos de los flags.
        // Pero es mejor que el esclavo siempre incluya el estado en la respuesta (ej. en el padding)
    }
    else if (response.type == TYPE_SYSTEM) {
        globalSensorData.calibrationK        = response.payload.system.K_fixed;
        globalSensorData.calibrationTau      = response.payload.system.tau_fixed;
        globalSensorData.calibrationProgress = response.payload.system.progress;
        globalSensorData.calibrationValid    = response.payload.system.calibration_valid;
        globalSensorData.lastSlaveState      = response.payload.system.state; // ¡Importante!
    }
...
void SPIMaster::testCommunication() {
    ...
    bool success = performSpiExchange(testCmd);
    ...
    success = performSpiExchange(testCmd);
    if (success) {
        Serial.println("  ✅ OK");
